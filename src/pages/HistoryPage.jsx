...
function getDateStyle(avgScore, isSelected) {
  if (isSelected) {
...
function buildCalendarCells(currentDate) {
  ...
  const startOffset = firstDay.day(); // 0 = 日
...
export default function HistoryPage() {
      ...
      try {
        ...
        results.forEach((res) => {
          ...
          history.forEach((item) => {
            const d = item.date;
            if (!accumulator[d]) accumulator[d] = { sum: 0, count: 0 };
        ...
        const avgMap = {};
        Object.entries(accumulator).forEach(([date, { sum, count }]) => {
          avgMap[date] = sum / count;
        });
        setDateScoreMap(avgMap);
      } catch (err) {
  ...
  const handleDateClick = async (dateStr) => {
    if (!(dateStr in dateScoreMap)) return;
    setSelectedDate(dateStr);
    setDaySignals([]);
    setLoadingDay(true);
    try {
      const res = await getSignalsByDate(dateStr);
      setDaySignals((res.data.data || []).map(mapSignal));
    } catch (err) {
  ...
  return (
    <div className="container-fluid py-4">
          ...
          <>
            ...
            >
              {cells.map((day, idx) => {
                ...
                const hasData = dateStr in dateScoreMap;
                const isSelected = selectedDate === dateStr;
                const style = hasData
                  ? getDateStyle(dateScoreMap[dateStr], isSelected)
                  : null;
            ...
            <div
              ...
              style={{ fontSize: 11, color: "#8ab4d4" }}
            >
              <span>
                <span style={{ color: "#26a69a" }}>■</span> 買進訊號（≥5）
              </span>
              <span>
                <span style={{ color: "#ffa726" }}>■</span> 觀望（0–4）
              </span>
              <span>
                <span style={{ color: "#ef5350" }}>■</span> 賣出訊號（&lt;0）
              </span>
            </div>
          </>
        )}
      ...
      {selectedDate && (
        <div>
          <h6 className="section-title">{selectedDate} 評分詳情</h6>
          ...
          {!loadingDay && daySignals.length === 0 && (
            <p className="text-secondary">無評分資料</p>
          )}
        </div>
      )}
    </div>
